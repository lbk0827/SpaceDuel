...
- 색은 슬롯 순서로 **모두에게 동일**하게 보인다: 파랑 · 빨강 · 노랑 · 초록. 내 색은 슬롯 순서에 따라 정해지므로(항상 파랑이 아니다) **내 캐릭터 머리 위에 삼각 표식**이 붙고 HUD 도 `YOU(색이름)` 으로 표시된다. 맞으면 즉사, **마지막 생존자가 라운드 획득**, 전멸은 무득점. 라운드 시작 즉시 발사 가능.
- 탄이 상대에게 맞을 것 같으면 슬로모션.
- 스폰: 방의 네 모서리(2인이면 대각선).

## 아이템 (배달선)

`itemShipInterval`(기본 12초)마다 **배달 우주선**이 방을 가로지르며 아이템 상자를 떨어뜨린다. 몸으로 닿으면 획득. **아이템과 효과는 라운드마다 초기화**된다.

| 아이템 | 표시 | 효과 | 상한 |
|---|---|---|---|
| 배리어 | 하늘색 `B` | 피격을 **1회 막는다**(남은 횟수만큼 몸 주변 호로 표시) | 3 |
| 웨폰 SPD UP | 노란 `S` | 레이저 속도 **+35%/개** | 3 |
| 탄창 | 초록 `M` | 쿨다운 → **에너지(연사) 모드**, 최대 탄약 = 1+개수 | 3 (최대 4발) |

내가 가진 효과는 좌하단에 `BARRIER 3` / `SPD 1` / `MAG 1` 칩으로 표시된다.
