...
- 색은 슬롯 순서로 **모두에게 동일**하게 보인다: 파랑 · 빨강 · 노랑 · 초록. 내 색은 슬롯 순서에 따라 정해지므로(항상 파랑이 아니다) **내 캐릭터에는 점선 링 + 머리 위 삼각 표식**이 붙고 HUD 도 `YOU(색이름)` 으로 표시된다. 맞으면 즉사, **마지막 생존자가 라운드 획득**, 전멸은 무득점. 라운드 시작 즉시 발사 가능.
